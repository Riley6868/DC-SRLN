...
class Net(nn.Module):
    def __init__(self, seresnet50, resnet50):
        ...
        # print('*'*50)
        #
        # print(list(seresnet50.pretrained_model.children()))
        ...
        # self.layer4a = list(resnet50.children())[4]
    ...
    def forward(self, input):
        ...
        x = self.avgpool(x)
        x = x.reshape(x.size(0), -1)*0.9
        xd = nn.Dropout(p=0.5)(x)

        x1 = self.avgpoola(x1)
        x1 = x1.reshape(x1.size(0), -1)*0.8
        x1d = nn.Dropout(p=0.5)(x1)
        xda = torch.cat((xd, x1d), 1)
        xda = self.fc6(xda)
        feature2 = xda
