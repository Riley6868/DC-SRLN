...
from core import model, modelse, dataset
...
start_epoch = 1
save_dir = os.path.join(save_dir, datetime.now().strftime('SE50_%Y%m%d_%H%M%S'))
print(save_dir)
...
# define model
net = modelse.attention_net(topN=PROPOSAL_NUM)
if resume:
...
for epoch in range(start_epoch, 101):
    ...
    for i, data in enumerate(trainloader):
        ...
        part_loss = modelse.list_loss(part_logits.view(batch_size * PROPOSAL_NUM, -1),
                                    label.unsqueeze(1).repeat(1, PROPOSAL_NUM).view(-1)).view(batch_size, PROPOSAL_NUM)
        raw_loss = creterion(raw_logits, label)
        concat_loss = creterion(concat_logits, label)
        rank_loss = modelse.ranking_loss(top_n_prob, part_loss)
...
# save model
        ...
        torch.save({
            ...
            os.path.join(save_dir, 'seresnet50_%03d.ckpt' % epoch))
